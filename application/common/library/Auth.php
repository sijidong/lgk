...
class Auth
{
    ...
    protected $_token = '';
    //Token默认有效时长
    protected $keeptime = 249200;
    protected $requestUri = '';
    protected $rules = [];
    //默认配置
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','invitecode','email'];
...
//        }
    ...
    public function login($account, $password)
    {
        ...
        if ($user->status != 'normal') {
            if (!empty($user->mark_login)) {
                $this->setError($user->mark_login);
                return false;
            }
            $this->setError('Account is locked');
            return false;
